...
class LongCatImagePipelineImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor unpack_latents(const torch::Tensor& latents,
    ...
    int64_t orig_height = height;
    int64_t orig_width = width;
    height = 2 * (height / (vae_scale_factor * 2));
    width = 2 * (width / (vae_scale_factor * 2));
  ...
  torch::Tensor encode_prompt_qwen(std::vector<std::string>& prompt,
    ...
    int64_t content_non_pad_count = 0;  // for first batch, used for debug log
    for (int64_t i = 0; i < batch_size; ++i) {
      ...
      if (i == 0) {
        content_non_pad_count = non_pad_count;
      }
    ...
    int64_t pad_start_idx = prefix_len + content_non_pad_count;
    int64_t total_non_pad = prefix_len + content_non_pad_count + suffix_len;
