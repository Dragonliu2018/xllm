...
torch::Tensor pack_latents(torch::Tensor latents,
                           ...
                           int64_t height,
                           int64_t width) {
  auto packed_latents = torch::reshape(
      latents, {batch_size, num_channels_latents, height, 2, width, 2});
  packed_latents = torch::permute(packed_latents, {0, 2, 4, 1, 3, 5});
  packed_latents = torch::reshape(
      packed_latents, {batch_size, height * width, num_channels_latents * 4});
...
torch::Tensor unpack_latents(torch::Tensor latents,
  ...
  int64_t num_channels_latents = latents.size(-1) / 4;

  auto unpacked_latents = torch::reshape(
      latents, {
    batch_size, height  // vae_scale_factor, width // vae_scale_factor, 2, 2,
                        // num_channels_latents});
                    unpacked_latents =
                        torch::permute(unpacked_latents, {0, 5, 1, 3, 2, 4});
    unpacked_latents = torch::reshape(
        unpacked_latents, {batch_size, num_channels_latents, height, width});
...
torch::Tensor prepare_latent_image_ids(int64_t batch_size,
                                       int64_t height,
                                       int64_t width) {
    std::vector<int64_t> latent_image_ids;
    for (int64_t h = 0; h < height; ++h) {
      for (int64_t w = 0; w < width; ++w) {
        latent_image_ids.push_back(0);
        latent_image_ids.push_back(h);
        latent_image_ids.push_back(w);
      }
    }
    return torch::from_blob(latent_image_ids.data(),
                            {(int64_t)latent_image_ids.size() / 3, 3},
                            torch::kLong)
        .clone();
}
...
class LongCatImagePipelineImpl : public torch::nn::Module {
   public:
    LongCatImagePipelineImpl(const DiTModelContext& context) {
      ...
      scheduler_ = FlowMatchEulerDiscreteScheduler(
          context.get_model_context("scheduler"));
    ...
    void load_model(std::unique_ptr<DiTModelLoader> loader) {
      ...
      LOG(INFO)
          << "LongCat-Image model components loaded, start to load weights "
             "to sub models";
   ...
   private:
    ...
    std::vector<torch::Tensor> forward_(
        ...
        std::optional<std::vector<std::string>> negative_prompt_2 =
            std::nullopt,
