...
class ColaDLMPipelineImpl final : public torch::nn::Module {
 public:
  ...
  DiTForwardOutput forward(const DiTForwardInput& input) {
    ...
    // Note: per-request device RNG randomization is handled one level up in
    // DiTWorkerImpl::step(), which seeds the device generator with a unique
    ...
    // OLMo tokenizer: pad_token = 100277
